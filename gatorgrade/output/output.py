...
def run_checks(  # noqa: PLR0912, PLR0913, PLR0915
    ...
    if len(failed_results) > 0:
        ...
        # add reminder about auto-hints if any were generated
        if auto_hint_engine is not None and any(
            ...
            remote_url = getattr(auto_hint_engine, "remote_url", None)
            if has_fallback and remote_url:
                # remote failed; note the unavailable URL
                rich.print(
                    f"\n[dim]→ Use auto-hints generated by "
                    f"{auto_hint_engine.model_id} "
                    f"to spark ideas.\n"
                    f"  Failed to use remote server at {remote_url}.[/]"
                )
            elif auto_hint_url:
                rich.print(
                    f"\n[dim]→ Use auto-hints generated by "
    ...
    # scores are displayed as in the failure case
        ...
        # add reminder about auto-hints if the engine was used
        # (even though all checks passed, the engine was loaded)
